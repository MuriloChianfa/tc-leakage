...
#
# OpenVPN e2e test: verify netleak routes traffic through tun0,
...
# Test 1: Traffic routes through OpenVPN tunnel
assert_curl_ok "${SSH_KEY}" "${SSH_PORT}" "${IFACE}" "${URL}" \
    "traffic routes through ${IFACE}"

# Test 2: Kill-switch activates when tun0 goes down
...
# Test 3: Traffic recovers when tun0 comes back up
