...
#
# SoftEther VPN e2e test: verify netleak routes traffic through tap0,
...
# Test 1: Traffic routes through SoftEther tap interface
assert_curl_ok "${SSH_KEY}" "${SSH_PORT}" "${IFACE}" "${URL}" \
    "traffic routes through ${IFACE}"

# Test 2: Kill-switch activates when tap0 goes down
...
# Test 3: Traffic recovers when tap0 comes back up
